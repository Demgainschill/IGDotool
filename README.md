...
3. Regardless of whether the tool will perform a dependency check and install the tools required, the tool expects the user to download the tools himself manually if errors are caused since script checks and downloads only using apt package manager as of now (tools mentioned at the end section more package managers to be added newer releases).
...
A coffee worth donation would be great ☕ 
Btc address
